...
- **Wave 9 (2026-09-17): SF-Bok + World of Board Games** på ägarbeslut — SF-Boks Pokémon TCG är BUTIKSVARA (IN_STOCK = finns i fysisk butik, reservera), WOBG:s kategori var TOM (allt "Utgått"); detaljer i `scraping-restock.md`. **56 butiker, 44 bevakade** → **Aquitaz RETIRERAD 2026-09-21** (ägarbeslut, `retire-store.ts`: 604 offers bort, källa+retailer av, 5 exklusiva dolda) ⇒ **43 bevakade**. `AquitazAdapter` kvar som gravsten.
- **DISCORD-INLÄGGET BÄR "REK. PRIS" SEDAN 2026-09-21**: `Köp`-fältet ("🌐 Online" / "🏬 Endast i butik") är BORTTAGET 2026-09-22 (ägarbeslut) — butikskanalen säger det redan; `RawProductData.storeOnly` (SF-Bok `buttonState` 3/4, Webhallen `web: 0` + butikssaldo) styr bara routing + butikscopyn, lagerdomen är oförändrad. `Rek. pris` = `Product.msrpOre` (ägaren sätter via `scripts/set-msrp.ts`, ingen adminyta) eller kategoridefault i `src/lib/msrp.ts` (TOM tills ägaren fyller — aldrig påhittat), löst vid ruttexporten (`RouteEntry.msrpOre`, lanen är DB-fri ⇒ kör `restock-routes-export` efter ändring). Domen `msrpDelta` (ren, testad): ≤ rek = 🟢 + grön kant, > rek = 🔴 + röd kant — Discord färgar ingen löptext, därav emoji + kant. ⛔ MSRP rör ALDRIG rubrikpris/"Lägst"/statistik/larm.
- **BUTIKSVAROR HAR EGEN DISCORD-KANAL SEDAN 2026-09-22 (ägarbeslut)**: `"stores":"<id>"` i `DISCORD_RESTOCK_CHANNELS` tar ALLT med `storeOnly` — de övriga kanalerna är därmed rent ONLINE. Domen: `resolveRestockChannelId` (ren, testad); butikskanalen vinner över set/serie/språk OCH `"prices"`. ⛔ Utan `"stores"` är routingen oförändrad (fail closed hade tystat larm på en okonfigurerad server). Inlägget har egen copy: rubrik "Finns bara i butik: …", fält "Pris i butik" + **"I lager"** ("36 ex i 6 butiker", `StoreStock`/`formatStoreStock`), fot "ring butiken innan du åker" (källraden BORTTAGEN 2026-09-23, ägarbeslut). ⛔ Webhallens live-koll (`/api/product/{id}`, hittar påfyllningen ~50 min före sökindexet) MÅSTE skriva om `storeOnly`/`storeStock` ur live-svaret — annars postas en butikspåfyllning som online-restock (hände 30th Celebration 09-23). Webhallens butiker NAMNGES ur `/api/store/se` (`webhallen-stores.ts`, runtime-hämtad + 24 h cache, fail soft): "Fridhemsplan, Stockholm · 14 ex", störst först, max 4 rader + "+N butiker till"; EN butik ⇒ namnet står i löptexten. ⛔ Okänt saldo ⇒ ingen rad (aldrig "0 ex"); `displayCap` är ett VISNINGSTAK ⇒ "minst N ex"; ett okänt butiks-id får ingen rad men räknas i summan; ALDRIG ett gissat filialnamn. ⛔ **Läs butikens frontend-bundle innan du kallar något omöjligt** — endpointen stod i klartext i `js/app.*.js` efter att gissade URL:er gett 404. ⛔ Bara SF-Bok + Webhallen sätter `storeOnly` — ingen annan feed skiljer webblager från hyllsaldo. ⛔ Ingen uppdateringstakt utlovas (butikerna pollas i olika takt) och ALDRIG ett filialnamn — Webhallens saldon är namnlösa nycklar. ⛔ Pro-spegeln hoppar över butiksvaror: ingen korg att lägga i. **ONLINE OCH BUTIK ÄR TVÅ SPÅR sedan 2026-09-23 (ägarbeslut)**: en källa med `storeStatus` (bara Webhallen) diffas i lanen som huvudnyckeln = WEBBLAGRET (butiksvara ⇒ slut där) + `<url>#butik` = butikerna ⇒ påfyllning på båda ställena = ett inlägg i VARJE kanal ("Finns i butik: …" / "Går även att beställa i webbutiken" när webben också har den; den posten ger ingen hit). Nya `#butik`-nycklar seedas tyst per källa. DB-vägens `stockStatus` är orörd. Regler: `discord-restock.md`.
- **Butikssvansen (wave 7, 2026-09-06 — 54 butiker, 42 bevakade; Blindbox ur bevakningen 2026-09-15 på ägarbeslut, `scripts/retire-store.ts`; Rogerz + Pokexclusive UR LARMEN 2026-09-17 på ägarbeslut men kvar som pris-/lagerkällor — `scripts/set-restock-watch.ts --off`, ruttexport efteråt)**: ✅ Sweet Nerds (Nyehandel),
