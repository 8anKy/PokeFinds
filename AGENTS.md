...
- **BUTIKSVAROR HAR EGEN DISCORD-KANAL SEDAN 2026-09-22 (ägarbeslut)**: `"stores":"<id>"` i `DISCORD_RESTOCK_CHANNELS` tar ALLT med `storeOnly` — de övriga kanalerna är därmed rent ONLINE. Domen: `resolveRestockChannelId` (ren, testad); butikskanalen vinner över set/serie/språk OCH `"prices"`. ⛔ Utan `"stores"` är routingen oförändrad (fail closed hade tystat larm på en okonfigurerad server). Inlägget har egen copy: rubrik "Finns bara i butik: …", fält "Pris i butik" + "Källa: Butikens lagersaldo", fot "ring butiken innan du åker". ⛔ Ingen uppdateringstakt utlovas (butikerna pollas i olika takt) och ALDRIG ett filialnamn — Webhallens saldon är namnlösa nycklar. ⛔ Pro-spegeln hoppar över butiksvaror: ingen korg att lägga i. Regler: `discord-restock.md`.
