...
import { normalizeFeed, type FeedDocument } from "../src/lib/feed";
...
const doc = normalizeFeed({ generatedAt: new Date().toISOString(), news: [...news, ...current.news.filter((n) => n.lane !== "curated")], events: current.events });
writeFileSync(file, JSON.stringify(doc));
console.log(`skrev ${file}: ${doc.news.length} nyheter, varav ${news.length} curated:`);
