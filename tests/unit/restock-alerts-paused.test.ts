...
vi.mock("@/lib/db", () => ({
  prisma: {
    ...
    watchlistItem: {
      findMany: (...a: unknown[]) => watchlistFindMany(...a),
    ...
    // Prislarmets köpbara offer (i lager + direktlänk + > 0 kr).
    offer: {
      findMany: vi.fn().mockResolvedValue([
        { id: "o1", productId: "p1", price: 89000, url: "https://butik.se/products/pitch-black-etb", retailerId: "r1", retailer: { name: "Butiken" } },
      ]),
    },
...
describe("pausat läge", () => {
  ...
  it("rör INTE prislarmen — de har en egen flagga", async () => {
    ...
    process.env.PRICE_ALERTS_PAUSED = "0";
    productFindUnique.mockResolvedValue({ id: "p1", title: PRODUCT.title, slug: PRODUCT.slug, hiddenAt: null, lowestPriceOre: 95000 });
    watchlistFindMany.mockResolvedValue([{ id: "w1", userId: "u1", targetPrice: 90000, priceAlertFiredOre: null }]);
    const r = await checkPriceAlerts("p1");
  ...
  it("och åt andra hållet: pausade prislarm rör inte restock-larmen", async () => {
    ...
    const p = await checkPriceAlerts("p1");
